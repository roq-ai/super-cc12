...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'manager', 'cashier', 'inventory-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MerchantInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/merchants` : null),
    () => getMerchants({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Merchants',
      path: isTenantUser ? `/merchants/view/${data?.[0]?.id}` : '/merchants',
      entity: 'merchant',
      service: AccessServiceEnum.PROJECT,
      icon: FiShoppingBag,
    },
    { name: 'Inventories', path: '/inventories', entity: 'inventory', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
    ...
    { name: 'Payments', path: '/payments', entity: 'payment', service: AccessServiceEnum.PROJECT, icon: FiCreditCard },
