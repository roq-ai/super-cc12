...
import { getMerchants, deleteMerchantById } from 'apiSdk/merchants';
import { MerchantInterface } from 'interfaces/merchant';

type ColumnType = ColumnDef<MerchantInterface, unknown>;
...
export function MerchantListPage(props: MerchantListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getMerchants({
        relations: ['user', 'inventory.count', 'loyalty_program.count', 'order.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MerchantInterface>>(
    () => `/merchants?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MerchantInterface) => {
    if (hasAccess('merchant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/merchants/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('loyalty_program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.loyalty_program,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/merchants/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('merchant', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/merchants/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('merchant', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
