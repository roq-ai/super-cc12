...
import { getLoyaltyPrograms } from 'apiSdk/loyalty-programs';
...
function DashboardPage() {
  ...
  const loyaltyProgramStat = useSWR<PaginatedInterface<LoyaltyProgramInterface>>(
    () => `/loyalty-programs`,
    () => getLoyaltyPrograms({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: loyaltyProgramStat?.data?.totalCount,
      isLoading: loyaltyProgramStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Orders',
