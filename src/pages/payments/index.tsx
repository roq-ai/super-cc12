  ...
  const fetcher = useCallback(
    async () =>
      getPayments({
        ...
        order: params.order,
        searchTermKeys: ['payment_method.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PaymentInterface>>(
    () => `/payments?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.payment_date ? format(parseISO(record?.payment_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'payment_amount', header: 'payment_amount', accessorKey: 'payment_amount' },
